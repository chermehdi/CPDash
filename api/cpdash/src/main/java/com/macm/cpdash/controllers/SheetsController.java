...
public class SheetsController {
	...
	public ResponseEntity<List<Sheet>> getAllByUser(@PathParam(value = "userId") Long userId) {
		...
		return new ResponseEntity<List<Sheet>>(user.getSheets().stream().map(Sheet::new).collect(Collectors.toList()),
				HttpStatus.ACCEPTED);
	}

	@GetMapping(path = "/{sheetId}")
	public ResponseEntity<Sheet> getById(@PathParam(value = "sheetId") Long sheetId) {
		return new ResponseEntity<Sheet>(new Sheet(sheetRepository.findOne(sheetId)), HttpStatus.ACCEPTED);
	}
	...
	public ResponseEntity<Sheet> create(@RequestBody Sheet sheet, @PathParam(value = "userId") Long userId) {
		...
		sheetEntity.setSheetHash("MaHashmaWalo");

		sheetEntity = sheetRepository.save(sheetEntity);
		return new ResponseEntity<Sheet>(new Sheet(sheetEntity), HttpStatus.ACCEPTED);
	}
}
